...
export default function ResetViaEmailStep2() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem("resetTokenStep1")
      const { data } = await axios.post(
        "/api/v1/auth/forget-password-verification",
        { verificationCode: otp },
        { headers: token ? { Authorization: `Bearer ${token}` } : undefined },
      )
      if (data?.token) sessionStorage.setItem("resetTokenStep2", data.token)
      toast.success(`OTP verified successfully for ${email}`)
    ...
    <ForgotPasswordLayout stepTitle="Step 2: Verify OTP">
      <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm">
          ...
          <span className="font-semibold text-blue-700">{email}</span>. Please enter it below to proceed.
        </div>
        ...
        <div className="text-center mt-4">
          <NavLink to="/ResetViaEmail" className="text-gray-600 hover:underline flex items-center justify-center gap-1">
            ← Back to Email Step
          </NavLink>
        </div>
      </div>

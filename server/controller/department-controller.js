...
const res = require("express").response // Declare res variable
...
const assignWorkerToReport = async (req, res) => {
  try {
    ...
    console.log(role)
    if (!["department-admin", "state-admin", "village-admin", "district-admin"].includes(req.userInfo.role)) {
      return res.status(403).json({
        success: false,
        message: "Access denied. Only authorized admins can assign workers.",
      })
    }

    // Scope checks for village/district admins
    if (req.userInfo.role === "village-admin") {
      const adminVillage = (req.userInfo.villageName || "").toLowerCase()
      const reportVillage = (report.village || "").toLowerCase()
      if (adminVillage && reportVillage && adminVillage !== reportVillage) {
        return res.status(403).json({
          success: false,
          message: "Access denied. Report not in your village.",
        })
      }
    }
    if (req.userInfo.role === "district-admin") {
      const adminDistrict = (req.userInfo.assignedDistrict || req.userInfo.District || "").toLowerCase()
      const reportDistrict = (report.district || "").toLowerCase()
      if (adminDistrict && reportDistrict && adminDistrict !== reportDistrict) {
        return res.status(403).json({
          success: false,

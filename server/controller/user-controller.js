...
const registration = async (req, res) => {
  ...
  if (registerationAttemptsByUser.length > 5) {
    return res.status(429).json({
      success: false,
      message:
        "You have exceeded the maximum number of attempts (5). Please try again after 1 hour.",
    });
  }
...
const verifyOtp = async (req, res) => {
  try {
    ...
    if (!user.verificationCodeExpire || user.verificationCodeExpire > Date.now() + 10 * 60 * 1000) {
      return res.status(400).json({
        success: false,
